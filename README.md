...
#### A possilble theory of UEs: Token Drag
My working theory of why this works is simple. Low run tokens are 'dragging' the model back to an older state. Think of it like this, an old embedding that doesn't get seen very often is 'fit' to a version of the weights that is behind the current model. When an example happens with that embedding the embedding gets dragged closer to the current model but similarly the rest of the model gets 'dragged' back to where that embedding is at. This means embeddings have two jobs: store knowledge AND 'fit' what the model expects from an embedding. By prividing the sacrificial training network that predicts embeddings instead of looking them up the model doesn't get dragged back nearly as much since the network is effectively separating the prediction of how to interface with the model while the embeddings can just store knowledge and not worry about also trying to fit the model well. Removing the 'fit' job allows the embeddings to pack a lot more information since they don't have to constantly adjust to a changing model. At least that is the theory. There is definitely evidence of token drag though. If you apply UEs to embeddings that don't have low frequency members (like positional embeddings) there is no real advantage. This implies that low frequency token interactions really are what are being improved in some way.
